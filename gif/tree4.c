...
BiTNode *CreateBiTree(BiTNode *tree,int color);
...
void shixinDemo(BiTNode *tree,int lcolor)
{
  int mradius;				/* Maximum radius allowed	*/
  char c;
    ...
    if(tree==NULL) 
	{
		...
		setcolor( tree->color);	/* Randomly select a color	*/
		setfillstyle( 1,tree->color);	
		pieslice( tree->x, tree->y,0,360 ,mradius );
		setcolor(15); 
		settextstyle(0,0, 1); 
		outtextxy(tree->x,tree->y, tree->data); 	
	
        shixinDemo(tree->lchild,lcolor);  
        shixinDemo(tree->rchild,lcolor);  
...
{  
    ...
    {  
        ...
        tree->lchild = CreateBiTree(tree->lchild,color);  
        tree->rchild = CreateBiTree(tree->rchild,color);  
    }  
